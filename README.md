...
| Retrieval | `/cmo:recall` skill (model- and user-invocable) | Tiered lazy search: index → handoff → ranked synonym-aware search (`scripts/search.js`, glossary-expanded, weak matches labeled) → `Grep` the journal → `Grep` spill files. The search corpus also covers a [Trolly](https://github.com/gritbox/trolly) `journey/` tree when present (specs, blueprints, ADRs; `journey/archive/` excluded as stale-by-design; journey hits quota-capped so they can't crowd out memory). On a miss the model retries with its own synonyms — the searcher is the query expander |
| Durable facts | `/cmo:remember` skill | Curates `decisions.md`/`index.md` with supersede-in-place and a line budget, plus glossary aliases (recall insurance: the write-time model records the synonyms it can foresee, so recall-time expansion has something to expand). Glossaries are multi-source: `.cmo/glossary.md`, plus `journey/glossary.md` read directly in its own line shape on Trolly projects (one canonical file, no mirroring, heads deduplicated across sources), plus `CMO_GLOSSARY_PATHS` extras |
...
| `CMO_GLOSSARY_PATHS` | — | Comma-separated extra glossary files (relative to the project root) read alongside `.cmo/glossary.md` and an auto-discovered `journey/glossary.md` |
| `TROLLY_DIR_JOURNEY` | `journey` | Name of the Trolly journey directory searched by `/cmo:recall` (matches Trolly's own override) |
